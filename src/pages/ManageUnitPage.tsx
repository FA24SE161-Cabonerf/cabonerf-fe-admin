...
const ManageUnitPage = () => {
  ...
  const [unitGroupId, setUnitGroupId] = useState(parseInt(searchParams.get('unitGroupId') || '0'))
  const [searchTerm, setSearchTerm] = useState(searchParams.get('search') || '')

  const { data: unitGroups, isLoading: isLoadingUnitGroups } = useUnitGroups()
  const { data: unitsData, isLoading: isLoadingUnits, error } = useUnits(page, pageSize, unitGroupId)
  ...
  const filteredUnits = unitsData?.data.listResult.filter(unit =>
  ...
  const handleNextPage = () => {
    setPage((prevPage) => Math.min(prevPage + 1, unitsData?.data.totalPage || prevPage))
  }

  if (isLoadingUnits || isLoadingUnitGroups) {
